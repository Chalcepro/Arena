...
func _setup_ui() -> void:
	...
	settings_ui.settings_cleared.connect(_on_settings_cleared)
...
func _on_settings_cleared():
	# Reset to defaults
	api_manager.set_provider("ollama")
	api_manager.set_model("qwen2.5-coder:3b")
	settings_ui.set_model("qwen2.5-coder:3b")
	
	# Force provider selection in UI
	var providers = api_manager.get_provider_list()
	var p_idx = providers.find("ollama")
	if p_idx >= 0: 
	...
	# Show feedback
	chat_ui.add_message("System", "Settings reset to defaults")
...
func _on_chunk_received(chunk: String) -> void:
	...
	chat_ui.update_streaming_message(sender, chunk)
...
func _load_settings() -> void:
	...
	if config.load("user://ai_assistant_settings.cfg") == OK:
		...
		var prov: String = config.get_value("ai_assistant", "provider", "ollama")
