...
@onready var muzzle_flash = $MuzzleFlash          # optional child node
@onready var weapon_mesh_holder = $WeaponMeshHolder  # holds the 3D model
...
func _process(delta):
	...
	# Reload input – guard against null weapon_data
...
func shoot():
	...
	if result:
		...
		# Check if we hit something with a "take_damage" method (e.g., a player)
		if result.collider.has_method("take_damage"):
			var hit_location = get_hit_location(result.position, result.normal)
			result.collider.take_damage(damage_dealt, hit_location)
...
func calculate_damage() -> int:
	if not weapon_data:
		return 0
	# Base damage; you can extend this with hit‑location multipliers later
	return weapon_data.damage


func get_hit_location(position: Vector3, normal: Vector3) -> String:
	# Placeholder – in a full game you would detect head/body/limbs
	return "body"
